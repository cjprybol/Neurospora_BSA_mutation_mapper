...
if [ ! -d "$BASE/GFF_FILTERED_BAM_SNPLESS" ];
        then
                mkdir "$BASE/GFF_FILTERED_BAM_SNPLESS"
                echo "> created directory $BASE/GFF_FILTERED_BAM_SNPLESS"
fi
...
OUT_DIR="$BASE/GFF_FILTERED_BAM_SNPLESS"
...
do
	...
	in_file=${f##*/}
	out_file=$(echo "$in_file" | sed -e 's/\.minus_mv_snps.sorted.*/\.gff_filtered/')
